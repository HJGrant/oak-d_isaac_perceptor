...
def load_voxel_data_from_csv(csv_filename):
    ...
    voxel_sizes = df[['VoxelSize_m']].values  # Each voxel may have different size
    
    # Extract voxel colors (normalize to [0, 1])
...
def process_folder(folder_path):
    ...
    for csv_file in csv_files:
        ...
        # Reduce data size if needed (downsampling)
        if len(voxel_centers) > 100000:
            voxel_centers = voxel_centers[::10]  # Take every 10th voxel
...
if __name__ == "__main__":
    folder_path = "extracted_data/box_0.025"  # Change this path
